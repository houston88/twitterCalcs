...
def main():
    ...
    print 'Inserted into mongo happiest_states, id: ' + str(rec_id)

    # now lets insert into historical data total happyness
    happiest_times = db.happiest_times
    # can we iterate through attrs?
    totalScore = 0
    for attr in happyState:
        #print attr
        if attr != 'parseDate' and attr != '_id' and attr != 'hashtags':
            state = happyState[attr]
            if 'score' in state and state['score'] != 'NA':
                totalScore += state['score']
    # now we have total for that record
    print 'Total score on ' + str(happyState['parseDate']) + ' : ' + str(totalScore)
    # lets try and insert into collection
    happiest_times.insert({'parseDate':happyState['parseDate'], 'score':totalScore})
